    ...
    try:
        for asset in data:
            ...
            timestamp = datetime.strptime(
                asset["quote"]["GBP"]["last_updated"], "%Y-%m-%dT%H:%M:%S.%fZ"
            )
...
def fetch_coins_data_from_db(conn):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM on_your_market_prices "
...
def ticker_search_alphavantage(query_term):
    ...
    url = (
        f"https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords="
...
def fetch_stocks_data_from_db(conn):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM on_your_market_prices " "WHERE coin_name IN ('IBM','VOO');"
    )
    data = cursor.fetchall()
    cursor.close()
